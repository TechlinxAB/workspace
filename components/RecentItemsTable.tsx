...
export function RecentItemsTable({ items }: RecentItemsTableProps) {
  return (
    <Card className="h-full overflow-hidden">
      <div className="mb-4">
        <p className="text-[15px] font-semibold text-ink">Recent Items</p>
          ...
          <thead>
            <tr className="border-b border-[rgba(18,18,18,0.08)]">
              {["Ref", "Date", "Type", "Amount/Impact", "Status"].map((heading) => (
                <th key={heading} className="px-6 py-3 text-[12px] font-semibold text-muted">
                  {heading}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.ref} className="border-b border-[rgba(18,18,18,0.06)]">
                <td className="px-6 py-4 text-sm font-semibold text-ink">{item.ref}</td>
                <td className="px-6 py-4 text-sm text-muted">{item.date}</td>
                <td className="px-6 py-4 text-sm text-ink">{item.type}</td>
                <td className="px-6 py-4 text-sm text-ink">{item.amountOrImpact}</td>
